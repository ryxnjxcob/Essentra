    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 px-4 pointer-events-none">
      <header className="w-full max-w-6xl glass dark:bg-[#121212]/80 rounded-full shadow-2xl shadow-black/5 dark:shadow-black/20 pointer-events-auto transition-all duration-500 border border-white/20 dark:border-white/10 backdrop-blur-xl">
        <div className="px-6 h-16 flex items-center justify-between">
          ...
          {user && (
            <div
              className="hidden md:flex flex-1 max-w-md mx-12 opacity-0 animate-fade-in"
              ...
              <>
                      ...
                      <div className="max-h-[60vh] overflow-y-auto p-2">
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((notif) => (
                            <div
                              key={notif.id}
                              className="p-3 mb-1 rounded-2xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors relative group cursor-default"
                            >
                              <div className="flex gap-3">
                                <div
                                  className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 shadow-sm ${notif.read ? "bg-gray-300 dark:bg-gray-700" : "bg-primary animate-pulse"}`}
                                />
                ...
                <div className="relative ml-2">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-3 w-3 text-muted-foreground transition-transform duration-300 ${showProfile ? "rotate-180" : ""}`}
                    />
                  </button>
                  {showProfile && (
