...
import { Image as ImageIcon, Type, Share2, ArrowLeft, BrainCircuit, Download, MousePointer2, Save, GripVertical, Moon, Sun } from 'lucide-react';
import { Board, Note, NoteType, User } from '../../types';
...
const BoardEditor: React.FC<BoardEditorProps> = ({ board, currentUser, onBack, onUpdateBoard, isDark, toggleTheme }) => {
  ...
  const addNote = (type: NoteType, content: string = '') => {
    const newNote: Note = {
      ...
      size: type === NoteType.TEXT ? { width: 280, height: 200 } : { width: 320, height: 240 },
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
        ...
        const type = file.type.startsWith('video') ? NoteType.VIDEO : NoteType.IMAGE;
    ...
    <div className="flex flex-col h-screen w-full bg-[#f8f9fa] dark:bg-[#050508] relative overflow-hidden">
      ...
      <div className="absolute top-6 left-6 right-6 z-40 flex justify-between items-start pointer-events-none">
         ...
         <div className="pointer-events-auto flex items-center gap-4">
            <button onClick={onBack} className="w-12 h-12 flex items-center justify-center bg-white dark:bg-[#18181b] rounded-full shadow-xl border border-black/5 dark:border-white/5 hover:scale-105 transition-all text-foreground">
            ...
            <div className="bg-white/80 dark:bg-[#18181b]/80 backdrop-blur-xl px-6 py-3 rounded-full shadow-xl border border-black/5 dark:border-white/5 flex flex-col justify-center min-w-[120px]">
                <h1 className="font-bold text-foreground text-sm leading-tight">{board.title}</h1>
         ...
         <div className="pointer-events-auto">
            <div className="bg-white/80 dark:bg-[#18181b]/80 backdrop-blur-xl p-1.5 rounded-full shadow-xl border border-black/5 dark:border-white/5 flex items-center gap-3">
                 ...
                 <div className="flex -space-x-2 px-2">
                    ...
                    <img key={collab.id} src={collab.avatar} alt={collab.name} className="w-8 h-8 rounded-full border-2 border-white dark:border-[#18181b]" title={collab.name} />
                    ))}
                </div>
                ...
                >
                    <div className={`absolute top-1 left-1 w-6 h-6 bg-white dark:bg-[#2a2a30] rounded-full shadow-md transform transition-transform duration-300 flex items-center justify-center ${isDark ? 'translate-x-6' : 'translate-x-0'}`}>
                        {isDark ? (
                 ...
                 <button onClick={() => alert(`Code: ${board.shareCode}`)} className="w-9 h-9 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 flex items-center justify-center hover:bg-purple-600 hover:text-white transition-all">
           ...
           <div className="bg-white dark:bg-[#18181b] rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] border border-white/10 animate-scale-in transform">
             <div className="p-8 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/5">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-2 font-medium ml-1">Transform text descriptions into visual workflows instantly.</p>
                </div>
                <button onClick={() => setShowFlowchartModal(false)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                    <div className="text-muted-foreground hover:text-foreground font-bold text-sm">Close</div>
                </button>
             </div>
               ...
               {!generatedCode ? (
                 <div className="relative group">
                    ...
                    <div className="absolute bottom-6 right-6 flex items-center gap-2">
                        <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{flowchartInput.length} chars</span>
                    </div>
                 </div>
               ) : (
                 <div className="bg-gray-50 dark:bg-black/20 p-6 rounded-2xl border border-gray-200 dark:border-white/10 relative h-full min-h-[300px] flex flex-col">
                     <div className="flex items-center justify-between mb-4">
                        <p className="text-xs font-mono text-purple-500 uppercase tracking-wider font-bold">Mermaid Syntax Preview</p>
                        <button onClick={() => setGeneratedCode('')} className="text-xs font-bold text-muted-foreground hover:text-primary underline decoration-2 underline-offset-2">Edit Prompt</button>
                     </div>
