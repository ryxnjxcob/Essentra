...
import { Plus, MoreVertical, Users, LogIn, Search, Grid, List, LayoutGrid, FolderPlus, Share2, Trash2, Copy, Check, FileText, Palette } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 animate-fade-in min-h-screen">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-12 gap-8">
        <div className="space-y-1">
          <h1 className="text-4xl md:text-5xl font-extrabold text-foreground tracking-tight">
             {getGreeting()}, <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-400">{user.name.split(' ')[0]}</span>
          </h1>
          <p className="text-muted-foreground text-lg font-medium">Let's create something impactful today.</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8 items-center">
          ...
          <div className="flex items-center bg-white dark:bg-white/5 p-1.5 rounded-2xl border border-gray-200 dark:border-white/10">
             <button onClick={() => setViewType('grid')} className={`p-2.5 rounded-xl transition-all ${viewType === 'grid' ? 'bg-gray-100 dark:bg-white/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
                <LayoutGrid className="w-4 h-4" />
             </button>
             <button onClick={() => setViewType('list')} className={`p-2.5 rounded-xl transition-all ${viewType === 'list' ? 'bg-gray-100 dark:bg-white/10 text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      {filteredBoards.length > 0 ? (
        <div className={`grid ${viewType === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} gap-6`}>
            ...
            >
                {/* Thumbnail Area */}
                <div className={`${viewType === 'grid' ? 'h-52 w-full' : 'h-full w-48 shrink-0'} bg-gray-50 dark:bg-black/20 relative overflow-hidden rounded-t-3xl ${viewType === 'list' && 'rounded-l-3xl rounded-tr-none'}`}>
                    ...
                    {board.type === 'notepad' ? (
                        ...
                        // Canvas Thumbnail
                        <>
                         <div className="absolute inset-0 bg-grid-slate-200/60 dark:bg-grid-slate-800/40 [mask-image:linear-gradient(to_bottom,white,transparent)]" style={{ backgroundSize: '24px 24px' }} />
                         {board.notes.length > 0 && board.notes.some(n => n.type === 'image') ? (
                             <img src={board.notes.find(n => n.type === 'image')?.content} className="w-full h-full object-cover opacity-80 transition-transform duration-700 group-hover:scale-105 group-hover:opacity-100" alt="thumbnail" />
                         ) : (
                    ...
                    <div className="absolute top-4 left-4 px-2.5 py-1 rounded-full bg-white/90 dark:bg-black/50 backdrop-blur text-[10px] font-bold uppercase tracking-wider border border-black/5 dark:border-white/10 text-muted-foreground flex items-center gap-1.5">
                        {board.type === 'notepad' ? <FileText className="w-3 h-3" /> : <Palette className="w-3 h-3" />}
                        {board.type}
                    </div>
                ...
                {/* Info Area */}
                <div className={`p-6 flex-1 flex flex-col justify-between ${viewType === 'list' ? 'py-0' : ''}`}>
                    <div className="w-full">
                        <div className="flex justify-between items-start w-full relative">
                            <h3 className="text-xl font-bold text-foreground group-hover:text-primary transition-colors line-clamp-1 pr-8">{board.title}</h3>
                    ...
                    <div className={`flex items-center justify-between ${viewType === 'grid' ? 'mt-6' : 'mt-2'}`}>
                        <div className="flex -space-x-2">
                            {board.collaborators.slice(0, 4).map((c) => (
                                <img key={c.id} src={c.avatar} className="w-8 h-8 rounded-full border-2 border-white dark:border-[#151518]" alt={c.name} />
                            ))}
                        </div>
                    </div>
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center animate-fade-in">
            ...
            <p className="text-muted-foreground max-w-md mb-8">Try adjusting your search or create a new canvas to get started.</p>
            {searchQuery && (
                <button 
      ...
      {showCreateModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-white dark:bg-[#18181b] rounded-3xl p-8 w-full max-w-2xl shadow-2xl border border-white/10 transform transition-all animate-scale-in">
                  <h3 className="text-2xl font-bold text-foreground mb-2">Create New Project</h3>
                  <p className="text-muted-foreground text-sm mb-8">Choose how you want to capture your ideas.</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                      ...
                      >
                          <div className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${selectedType === 'canvas' ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-white/10 text-muted-foreground'}`}>
                                <Palette className="w-6 h-6" />
                          </div>
                          <h4 className="text-lg font-bold text-foreground mb-1">Visual Canvas</h4>
                          <p className="text-sm text-muted-foreground">Infinite whiteboard for flowcharts, images, and spatial organization.</p>
                      </div>
                      ...
                      >
                          <div className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${selectedType === 'notepad' ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-white/10 text-muted-foreground'}`}>
                                <FileText className="w-6 h-6" />
                          </div>
                          <h4 className="text-lg font-bold text-foreground mb-1">Text Document</h4>
                          <p className="text-sm text-muted-foreground">Distraction-free markdown editor for writing and documentation.</p>
                      </div>
                  </div>

                  <div className="space-y-6">
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Project Title</label>
                        <input 
                            type="text"
                            placeholder={selectedType === 'canvas' ? "e.g. Q4 Strategy Roadmap" : "e.g. Meeting Notes"}
      ...
      {showJoinModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-white dark:bg-[#18181b] rounded-3xl p-8 w-full max-w-md shadow-2xl border border-white/10 transform transition-all animate-scale-in">
                  ...
                  <h3 className="text-2xl font-bold text-foreground mb-2">Join a Session</h3>
                  <p className="text-muted-foreground text-sm mb-8">Paste the invite code to collaborate.</p>
      ...
      {showShareModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-white dark:bg-[#18181b] rounded-3xl p-8 w-full max-w-md shadow-2xl border border-white/10 transform transition-all animate-scale-in">
                  ...
                  <h3 className="text-2xl font-bold text-foreground mb-2">Collaborate</h3>
                  <p className="text-muted-foreground text-sm mb-6">Share this code with your team to invite them to <span className="font-bold text-foreground">{showShareModal.title}</span>.</p>
                  
                  <div className="relative group cursor-pointer" onClick={() => handleCopyCode(showShareModal.shareCode)}>
                  ...
                  <p className={`text-center text-xs font-bold mt-3 transition-opacity ${copied ? 'text-green-500 opacity-100' : 'opacity-0'}`}>
                      Copied to clipboard!
                  </p>
